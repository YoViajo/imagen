...
# Procesar los directorios y obtener los conteos por año y cámara
...
# Ordenar el conteo global por cámara en orden descendente
sorted_cameras <- sort(global_camera_counts, decreasing = TRUE)

# Abrir el archivo de texto para guardar la salida
sink("conteo de fotos x cámara.txt")

# Mostrar el conteo de fotos para todas las cámaras
cat("\nConteo de fotos por cámara:\n")
for (camera_code in names(sorted_cameras)) {
  if (sorted_cameras[camera_code] > 0) {  # Eliminar las cámaras con conteo 0
    cat(paste("  ", camera_codes[[camera_code]], ": ", sorted_cameras[camera_code], "\n"))
  }
}
