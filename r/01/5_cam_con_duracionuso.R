...
# Función para calcular la duración en años, meses y días
calc_duration <- function(days) {
  years <- floor(days / 365.25)
  remaining_days <- days - (years * 365.25)
  months <- floor(remaining_days / 30.44)
  remaining_days <- round(remaining_days - (months * 30.44))
  return(list(years = years, months = months, days = remaining_days))
}
...
# Calcular la duración para cada cámara y guardarla en un data.frame
durations <- data.frame(Camera = character(), Duration = numeric())
for (camera_code in names(dates_for_all_cameras)) {
  ...
  if (length(dates) > 0) {
    ...
    duration_days <- as.numeric(difftime(max_date, min_date, units = "days"))
    durations <- rbind(durations, data.frame(Camera = camera_code, Duration = duration_days))
  }
}

# Ordenar el data.frame en orden descendente según la duración
durations <- durations[order(-durations$Duration),]

# Tomar las 5 cámaras con la mayor duración
top_5_cameras <- head(durations, 5)
...
sink("salida/5 cámaras con más duración uso.txt")

# Mostrar las 5 cámaras con la mayor duración
cat("Las 5 cámaras con más duración de uso son:\n")
for (i in 1:nrow(top_5_cameras)) {
  camera_code <- top_5_cameras$Camera[i]
  duration_days <- top_5_cameras$Duration[i]
  duration <- calc_duration(duration_days)
  cat(paste(camera_codes[[camera_code]], " - ", duration$years, "años ", duration$months, "meses ", duration$days, "días\n"))
}
