...
sink("salida/cámaras con duración uso.txt")

# Mostrar la duración para todas las cámaras en orden descendente
cat("Duración de uso para todas las cámaras (orden descendente):\n")
for (i in 1:nrow(durations)) {
  camera_code <- durations$Camera[i]
  duration_days <- durations$Duration[i]
  duration <- calc_duration(duration_days)
  
  # Solo incluir cámaras con una duración de uso no nula
