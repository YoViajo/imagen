...
sink("salida/rango_fechas_cámaras.txt")

# Mostrar el rango de fechas para cada cámara
for (camera_code in names(dates_for_all_cameras)) {
  ...
  if (length(dates) > 0) {
    ...
    formatted_min_date <- gsub("\\.", "", format(min_date, "%Y-%b-%d"))
    formatted_max_date <- gsub("\\.", "", format(max_date, "%Y-%b-%d"))
    cat(paste(camera_codes[[camera_code]], " - Entre ", formatted_min_date, " y ", formatted_max_date, "\n"))
  }
}
